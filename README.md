...
deliveries are 6.8% of all orders and 32.5% of 1-2 star reviews.

No small group explains them. The worst 17 sellers account for 4.6% of late deliveries once
each is measured against their own routes, and 11.3% even at the loosest threshold I tried.
Geography and demand peaks explain more.
...
There is an [interactive dashboard](https://public.tableau.com/app/profile/mariana.stepaniuk/viz/OlistDeliveryPerformance_17897417736350/Deliverypromise)
for the delivery-promise and seller parts.

## Dashboard

[Interactive version on Tableau Public](https://public.tableau.com/app/profile/mariana.stepaniuk/viz/OlistDeliveryPerformance_17897417736350/Deliverypromise)

![Delivery promise](images/dashboard_delivery.png)

![Sellers](images/dashboard_sellers.png)

All transformations live in the notebooks. Notebook 05 writes a star schema to
`data/bi/` and the dashboard reads that, so the BI layer does no computation of its own.
...
**[05_export_for_bi](notebooks/05_export_for_bi.ipynb)** — star schema for the dashboard:
`fact_orders`, `dim_seller`, `dim_date`
...
Not the sellers. Measured against the average late rate on their own destinations, 17 sellers
carry 298 of 6,534 late deliveries, 4.6%. At the loosest threshold it reaches 11.3% and no
further. Category explains as little, 4.2% to 8.1%.
...
`groupby` is not worth having, and my first RandomForest did not beat it. The biggest single
gain came from matching the loss to the metric: switching squared error to absolute error
moved validation MAE from 5.09 to 4.52, more than any model or feature I tried.

**A better prediction does not give a shorter promise.** A prediction sits in the middle of the
distribution, so half the orders arrive after it. A promise needs a margin on top, and the
margin has to cover the worst orders, which are exactly what the model misses. R2 is 0.23, and
orders that really take 30+ days get predicted at 10 to 20. Per-state margins help, from 10.3
days in SP to 30.2 in CE, but not enough to get under Olist's 22.4.

## What I would recommend

1. **Widen the promise before known peaks.** November 2017 ran 7,288 orders against 4,478 the
...
3. **SLA for the 17 flagged sellers.** A small lever at 4.6% of late orders, but the list is
...
**One seller per order.** Multi-seller orders are attributed to a single seller, using the rule
from notebook 01. Attributing such an order to every seller on it inflates the seller share:
the late order counts once per seller on top and once in the total below.
...
`data/raw/` and run the notebooks in order. 01 builds `data/clean/clean.parquet`, which 02, 03
and 04 all read. 04 works from the raw CSVs through DuckDB and takes only the seller
attribution from 01, so the two notebooks cannot drift apart.